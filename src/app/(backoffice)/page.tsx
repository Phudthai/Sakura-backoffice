...
export default function DashboardPage() {
  ...
  const cards = [
    ...
    { label: 'Processing', value: stats.processingOrders, icon: ShoppingCart, color: 'text-blue-600 bg-blue-50' },
    { label: 'Shipped', value: stats.shippedOrders, icon: Truck, color: 'text-indigo-600 bg-indigo-50' },
    { label: 'Completed', value: stats.completedOrders, icon: CheckCircle, color: 'text-green-600 bg-green-50' },
    { label: 'Revenue (THB)', value: `฿${formatPrice(stats.totalRevenueTHB)}`, icon: DollarSign, color: 'text-emerald-600 bg-emerald-50' },
    { label: 'Customers', value: stats.totalCustomers, icon: Users, color: 'text-purple-600 bg-purple-50' },
  ]
    ...
    <div>
      ...
      <div className="rounded-xl border border-card-border bg-white shadow-card">
        <div className="border-b border-card-border px-5 py-4">
          <h2 className="text-sm font-semibold text-sakura-900">Recent Orders</h2>
        </div>
            ...
            <tbody>
              {recentOrders.map((order) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-sakura-700">{order.customerName}</td>
