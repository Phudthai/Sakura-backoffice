...
export default function AuctionsPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [bidsRes, staffsRes] = await Promise.all([
        fetch('/api/backoffice/pending-bids'),
        fetch('/api/backoffice/staffs'),
      ])
  ...
  const handleReject = async (bidId: number) => {
    ...
    setActionLoading(true)
    try {
      const res = await fetch(`/api/backoffice/bids/${bidId}/reject`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const saveNote = async () => {
    ...
    setNoteSaving(true)
    try {
      const res = await fetch(`/api/backoffice/auction-requests/${editingNoteArId}/note`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-sakura-900 tracking-tight">Pending Bids</h1>
          <p className="text-sm text-muted-dark mt-0.5">Review and approve auction bid requests</p>
        </div>
                  ...
                  return (
                    ...
                    >
                      <td className="px-6 py-5 align-middle text-center w-36">
                        <span className="inline-flex items-center rounded-lg bg-sakura-100 px-2.5 py-1 font-mono text-xs font-medium text-sakura-800 max-w-full truncate" title={bid.auctionRequest?.externalId ?? undefined}>
                      ...
                      <td className="px-6 py-5 align-middle text-center w-24">
                        <div className="inline-flex flex-col items-center gap-0.5">
                          ...
                          {bid.staff && (
                            <p className="text-xs text-muted truncate max-w-full" title={bid.staff.name}>by {bid.staff.name}</p>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-5 align-middle text-center">
                        {bid.auctionRequest?.id != null && editingNoteArId === bid.auctionRequest.id ? (
                          ...
                          <button
                            type="button"
                            onClick={() => bid.auctionRequest?.id != null && startEditNote(bid.auctionRequest.id, bid.auctionRequest.note ?? null)}
                      ...
                      <td className="px-6 py-5 align-middle">
                        ...
                        {!isApproving && !isRejecting && isPending && (
                          <div className="flex gap-2">
                            <button
                              onClick={() => { cancelAction(); setApprovingId(bid.id) }}
                            ...
                            <button
                              onClick={() => { cancelAction(); setRejectingId(bid.id) }}
                  ...
                  <tr>
                    <td colSpan={10} className="px-6 py-16 text-center align-middle">
                      <p className="text-sakura-500 font-medium">
                        {filterUser ? `No bids found for "${filterUser}"` : 'No pending bids'}
                      </p>
                      <p className="text-sm text-muted mt-1">
                        {filterUser ? 'Try a different search term' : 'New bid requests will appear here'}
                      </p>
                    </td>
                  </tr>
                )}
